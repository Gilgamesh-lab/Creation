...
ecran.fill((43, 250, 250))
pygame.display.flip()
grille = [[0 for _ in range(70)]for _ in range(70)]
grille_bot = [[0 for _ in range(70)]for _ in range(70)]
...
grille_bot = cadre(af_x,af_y,"Changer le mot de passe du gestionnaire",24,grille_bot,'mdp')
grille_bot = cadre(af_x,af_y + 70,"Changer la question",32,grille_bot,'question')


if os.path.exists('Gestionnaire.json') == True:
    donnee = load("Gestionnaire")
    nb = donnee['nb']
    ...
    while l != mdp:
        ...
        if r == 2:
            ...
            if u == reponse:
                ...
                save(donnee,'Gestionnaire')
                break
                
    if w != 1:
        bonjour_phrase = randint(0,8)
    ...
    donnee = {}
    nb = randint(20,500) 
    donnee = {'nb': {} ,'mdp' : {} ,'question' : {} ,'reponse' : {} ,'mdp' : {} , 'bot': {} }
        ...
        if event.type == pygame.MOUSEBUTTONUP:
                ...
                if len(donnee['dico']) == 0:
                    ...
                    while u:
                        ...
                        if grille_liste[f[0]][f[1]] != 0 and  grille_liste[f[0]][f[1]] != 'home' :
                            copier(t[grille_liste[f[0]][f[1]]-1])
                            afficher(f'{t[grille_liste[f[0]][f[1]]-1]} as bien été copié dans le presse-papier',ecran,50,350)
                            sleep(2)
                            ecran.fill((43, 250, 250))
                            t = ''
                            for cle, valeur in donnee['dico'].items():
            ...
            if grille[a][b] == 3: #nouveau
                ...
                if l == True:
                    ...
                    while  cryptage(l,nb)  in donnee['dico']: 
            ...
            if grille[a][b] == 4: #changer
                ...
                if l == True:
                    ...
                    while not cryptage(l,nb)  in donnee['dico']: 
                        ...
                        if l != True:
                            l = correct(l)
                            if cryptage(l,nb) in donnee['dico']:
                                m = clavier( 'Entrer le nouveau mot de passe : ',ecran,af_x,af_y,False,True)
                                if m == True:
                                    pass  
                                else:
                                    m = correct(m)
                                    donnee['dico'][cryptage(l,nb)] = cryptage(m,nb)
                                    save(donnee,'Gestionnaire')
            ...
            if grille[a][b] == 5: #mdp
                ...
                if t == True:
                    ...
                    while not cryptage(t,nb) in donnee['dico']:
                        t = clavier('Nom de  mot de passe non reconnue , réessayer :  ',ecran,af_x,af_y,False,True )
                        if t == True :
                            break
                    if t != True:
                        t = correct(t)
                        if cryptage(t,nb) in donnee['dico']:
                            s = decryptage(str(donnee['dico'][cryptage(t,nb)]),nb) 
                            copier(s)
                            afficher(f'Le mot de passe pour {t} as été copié dans le presse papier',ecran,af_x,af_y)
                            sleep(2)
                menu(ecran)
            if grille[a][b] == 6:#créer
                l = clavier('Entrer le nom nouveau mot de passe : ',ecran,af_x,af_y,False,True)
                if l == True:
                    pass
                else:
                    while  cryptage(l,nb) in donnee['dico']:
            ...
            if grille[a][b] == 9:#Paramètre
                ...
                while u:
                    f = souris()
                    if grille_bot[f[0]][f[1]] == 'mdp':
                        l = clavier('Entrez le mot de passe actuel du gestionnaire : ',ecran,af_x,af_y,False,True)
                        if l == True:
                            pass
                        else:
                            r = 1
                    ...
                    if grille_bot[f[0]][f[1]] == 'question' :
                        l = clavier('Entrez le mot de passe actuel du gestionnaire : ',ecran,af_x,af_y,False,True)
                        if l == True:
                            pass
                        else:
